...
create table if not exists public.remittance_batches (
  ...
  status text not null default 'prepared'
    check (status in ('prepared','deposited','credited','cancelled')),
...
begin
  if not exists (
    ...
    where schemaname='public'
      and tablename='remittance_batches'
...
grant execute on function public.next_remittance_number(text,text)
to authenticated;
...
begin
  ...
  v_name:=coalesce(
    nullif(trim(p_name),''),
    public.next_remittance_number(p_payment_method,p_kind)
  );
  ...
  loop
    ...
    select b.id,b.remittance_kind
    into v_existing_batch,v_existing_kind
    ...
    if v_existing_batch is not null then
      if v_existing_kind='cash_reserve'
        ...
        delete from public.remittance_items
...
grant execute on function public.create_remittance_batch(
  text,text,uuid[],text,date,text
) to authenticated;
...
begin
  ...
  if p_credited_date is not null and p_deposit_date is null then
    raise exception 'Une date de dépôt est nécessaire avant le crédit bancaire';
  end if;
...
grant execute on function public.update_remittance_batch(
  uuid,date,date,text
) to authenticated;
...
grant execute on function public.cancel_remittance_batch(uuid)
to authenticated;
...
grant execute on function public.update_cash_float_target(numeric)
to authenticated;
